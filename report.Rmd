...
```{r reliability_validity, include=FALSE}
...
Time for grading each document
...
## Reliability and Validity Word Exam v1

Percentage correct by rater on a basis of `r dim(raters_err)[1]*dim(raters_err)[2]`

```{r percentage error}
kable(perc_error, caption = "Percentage correct")
```

Total value of errors by human raters on a basis of `r sum(ag$points)*dim(raters_err)[2]`
...
## Reliability and Validity Word Exam v2
...
```{r reliability_validity 2, include=FALSE}
colnames(a) <- tolower(colnames(a))
colnames(g) <- tolower(colnames(g))
colnames(r) <- tolower(colnames(r))
colnames(ag) <- tolower(colnames(ag))
...
## Reliability and Validity Word Exam v3
...
```{r reliability_validity 3, include=FALSE}
colnames(a) <- tolower(colnames(a))
colnames(g) <- tolower(colnames(g))
colnames(r) <- tolower(colnames(r))
colnames(ag) <- tolower(colnames(ag))
...
## Reliability and Validity Word Exam All Exams

Percentage correct by rater on a basis of `r tot_dim`

```{r percentage error total}
perc_error <- NULL
points_error <- NULL
...
kable(perc_error, caption = "Percentage correct - All Exams")
```

Total value of errors by human raters on a basis of `r sum(ag$points)*dim(raters_err)[2]`

```{r points error total}
kable(points_error, caption = "Total score error - All Exams")

#kable(points_error/(sum(ag$points)*dim(raters_err)[2])*100, caption = "Percentage score error - All Exams")
